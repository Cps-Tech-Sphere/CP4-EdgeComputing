...
void handleLuminosity() {
    const int potPin = 34;
    int somaLuminosity = 0;
    int luminosidadeMedia = 0;
    
    for (int i = 0; i < 10; i++) {
        int sensorValue = analogRead(potPin);  // Leia o sensor dentro do loop
        int luminosity = map(sensorValue, 0, 4095, 100, 0);  // Mapeie o valor do sensor
        somaLuminosity += luminosity;  // Some os valores
        delay(200);  // Atraso entre as leituras
    }
    
    luminosidadeMedia = somaLuminosity / 10;  // Calcule a média
    String mensagem = String(luminosidadeMedia);  // Converta para String
    
    Serial.print("Valor da luminosidade: ");
    Serial.println(mensagem.c_str());
    
    MQTT.publish(TOPICO_PUBLISH_2, mensagem.c_str());  // Publique no MQTT
}